...
#include "utrx_app_eds_dispatcher.h"
#include "utrx_app_eds_dictionary.h"

/*
 * Define a lookup table for SAMPLE app command codes
 */
static const EdsDispatchTable_UTRX_APP_Application_CFE_SB_Telecommand_t SAMPLE_TC_DISPATCH_TABLE = {
    .CMD     = {.NoopCmd_indication          = UTRX_APP_NoopCmd,
            .ResetCountersCmd_indication = UTRX_APP_ResetCountersCmd,
            .ProcessCmd_indication       = UTRX_APP_ProcessCmd,
            .DisplayParamCmd_indication  = UTRX_APP_DisplayParamCmd},
    .SEND_HK = {.indication = UTRX_APP_SendHkCmd}};
...
void UTRX_APP_TaskPipe(const CFE_SB_Buffer_t *SBBufPtr)
{
    ...
    Status = EdsDispatch_UTRX_APP_Application_Telecommand(SBBufPtr, &SAMPLE_TC_DISPATCH_TABLE);
